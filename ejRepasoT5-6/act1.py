...
try:
    ...
    # Intentar acceder a la base de datos para verificar la conexión
    colecciones = db.list_collection_names()
    print("Conexión exitosa. Colecciones en la base de datos después de la creación de productos:")
    print(colecciones)
    ...
    productos = coleccion_productos.find({"precio": {"$gt": 300}}).limit(2).sort("precio", -1)
    for producto in productos:
        print(producto)

    # Selecciona un producto por su nombre y actualiza su precio y stock con update_one().
    coleccion_productos.update_one({"nombre": "Laptop Lenovo"}, {"$set": {"nombre": "Portátil Lenovo", "stock": 20}})
    ...
    coleccion_productos.update_many({"categoria": "Electrónica"}, {"$inc": {"stock": 10}})
